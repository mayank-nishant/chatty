    ...
    const contacts = await Promise.all(
      users.map(async (user) => {
        ...
        return {
          ...
          lastMessage: lastMessage ? lastMessage.text || (lastMessage.image ? "📷 Image" : "No messages yet") : null,
...
export const getChatPartners = async (req, res) => {
  try {
    ...
    for (const msg of messages) {
      const otherUser = String(msg.senderId._id) === String(loggedInUserId) ? msg.receiverId : msg.senderId;

      if (!map.has(String(otherUser._id))) {
        map.set(String(otherUser._id), {
          ...otherUser.toObject(),

          lastMessage: msg.text || (msg.image ? "📷 Image" : "No messages yet"),
    ...
    return res.status(200).json({
      success: true,
      chatPartners: Array.from(map.values()),
    });
  } catch (error) {
